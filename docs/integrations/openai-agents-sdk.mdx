...
将 [**Mem0**](https://github.com/mem0ai/mem0) 与 [OpenAI Agents SDK](https://github.com/openai/openai-agents-python) 集成，这是一个用于构建多代理工作流程的轻量级框架。这种集成使代理能够跨对话访问持久内存，从而增强上下文保留和个性化。

## 概述

1. 在 OpenAI 代理中存储和检索 Mem0 中的内存
2. 具有共享内存的多代理工作流程
3. 检索过去对话的相关记忆
4. 基于用户历史记录的个性化响应
...
在使用 OpenAI Agents SDK 设置 Mem0 之前，请确保您拥有：

1.安装所需的包：```bash
pip install openai-agents mem0ai
```
2. 有效的 API 密钥：
   - <a href="https://app.mem0.ai/dashboard/api-keys?utm_source=oss&utm_medium=integration-openai-agents-sdk" rel="nofollow">Mem0 API 密钥</a>
   - [OpenAI API 密钥](https://platform.openai.com/api-keys)

## 基本集成示例

以下示例演示了如何创建具有 Mem0 内存集成的 OpenAI 代理：```python
import os
...
### 1. 自动内存集成
- **基于工具的记忆**：代理使用功能工具来搜索和保存记忆
- **对话存储**：所有交互都会自动存储
- **上下文检索**：代理可以访问相关的过去对话

### 2. 多代理内存共享
- **共享上下文**：多个代理访问同一内存存储
- **专业代理**：使用共享内存创建特定于域的代理
- **无缝切换**：代理在切换过程中维护上下文
