...
将 Mem0 的内存功能与 OpenAI 的内置工具集成，以创建具有持久内存的 AI 代理。
...
将 Mem0 和 OpenAI API 密钥保存在 `.env` 文件中：```
MEM0_API_KEY=your_mem0_api_key
OPENAI_API_KEY=your_openai_api_key
```
从 <a href="https://app.mem0.ai/dashboard/api-keys?utm_source=oss&utm_medium=cookbook-openai-tool-calls" rel="nofollow">Mem0 仪表板</a>获取 Mem0 API 密钥。
...
将记忆与网络搜索相结合以获得最新的推荐：```javascript
const response = await openAIClient.responses.create({
    model: "gpt-5-mini",
    tools: [{ type: "web_search_preview" }, carRecommendationTool],
    input: `${getMemoryString(relevantMemories)}\n${userInput}`,
...
- <a href="https://app.mem0.ai/dashboard?utm_source=oss&utm_medium=cookbook-openai-tool-calls" rel="nofollow">Mem0 仪表板</a>
...
<卡组列={2}>
  <Card title="Agents SDK Tool with Mem0" icon="robot" href="/cookbooks/integrations/agents-sdk-tool">
    使用 Mem0 集成功能扩展 OpenAI Agents SDK。
  </卡>
